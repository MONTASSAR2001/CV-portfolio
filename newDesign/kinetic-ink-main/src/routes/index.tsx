...
import { ScatterText } from "@/components/portfolio/ScatterText";
import { WorkSection } from "@/components/portfolio/WorkSection";

const KineticHero = lazy(() =>
  import("@/components/portfolio/KineticHero").then((m) => ({ default: m.KineticHero })),
);
const ProgressShape = lazy(() =>
  import("@/components/portfolio/ProgressShape").then((m) => ({ default: m.ProgressShape })),
);
      ...
      <nav className="fixed top-0 left-0 right-0 z-30 border-b border-ink bg-paper/90 backdrop-blur">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between px-6 py-3">
          <a href="#top" className="text-display text-xl tracking-tight">
            R.KOVAC<span className="text-neon">.</span>
          </a>
      ...
      <section id="about" className="border-t-[3px] border-ink bg-paper">
        <div className="mx-auto grid max-w-[1600px] grid-cols-12 gap-6 px-6 py-20">
          ...
          <div className="col-span-12 md:col-span-9 md:col-start-4">
            ...
            >
              WRITING IS NOT DECORATION. IT IS INFRASTRUCTURE. HOVER ANY WORD.
            </ScatterText>
            <div className="mt-10 grid grid-cols-1 gap-8 md:grid-cols-2">
              <ScatterText as="p" className="text-serif text-xl leading-relaxed italic">
                Most brand copy is written to be approved. Mine is written to be read. There is a
                difference, and it shows up in every metric that matters.
              </ScatterText>
              <ScatterText as="p" className="text-serif text-xl leading-relaxed italic">
                I work with founders, editors, and marketing leads who want language that carries
                weight. Short engagements, deep involvement, no jargon, no fluff, no rewrites for
                the sake of them.
              </ScatterText>
            </div>
          </div>
