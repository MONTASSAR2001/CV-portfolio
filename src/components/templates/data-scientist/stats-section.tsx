...
export function StatsSection({ data }: { data?: PortfolioData }) {
  const STATS: Stat[] = data?.projects?.length
    ? [
        {
          icon: TrendingUp,
          ...
          suffix: '',
          note: `${data.projects.length} production-ready system${data.projects.length !== 1 ? 's' : ''} shipped`,
        },
        {
          icon: DollarSign,
          label: 'Technologies Used',
          end: data.skills?.length ?? 0,
          accent: true,
          note: 'Across ML, analytics, and engineering stacks',
        },
        {
          icon: Target,
          label: 'Roles Held',
          end: data.experience?.length ?? 0,
          note: 'Professional engagements across industries',
        },
        {
          icon: Users,
          label: 'Qualifications',
          end: data.education?.length ?? 0,
          accent: true,
          note: 'Degrees and certifications earned',
        },
      ]
    : DEFAULT_STATS;
  return (
