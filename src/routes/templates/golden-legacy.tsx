...
import { Header } from "@/components/templates/golden-legacy/Header";
import { ServiceCard } from "@/components/templates/golden-legacy/ServiceCard";
import type { PortfolioData } from "@/components/portfolio-builder/types";

export const Route = createFileRoute("/templates/golden-legacy")({
  head: () => ({
    meta: [
      ...
      <div className="relative mx-auto grid w-full max-w-7xl grid-cols-1 items-center gap-16 px-8 pt-32 lg:grid-cols-[1.1fr_1fr]">
        ...
        <div className="relative z-10">
          ...
          <h1 className="font-serif text-5xl leading-[1.05] text-ivory md:text-7xl lg:text-[5.5rem]">
            {data?.personalInfo?.name ? (
              <>
                {data.personalInfo.name}
                <br />
                <span className="italic text-gold">{data.personalInfo.role ?? "Counsel"}</span>
              </>
            ) : (
              <>
                Counsel of
          ...
          <p className="mt-10 max-w-lg text-base leading-relaxed text-muted-foreground">
            {data?.personalInfo?.bio ?? "For more than a century, Ashford & Vale has advised sovereign institutions, private estates, and the world's most discerning enterprises on matters of consequence."}
          </p>
...
const DEFAULT_SERVICES = [
  {
    number: "I",
    title: "Corporate Law",
    description: "Mergers, acquisitions, and governance for multinational institutions. Strategic counsel through boardrooms and cross-border transactions of the highest complexity.",
    icon: <Landmark className="h-6 w-6" strokeWidth={1} />,
  },
  {
    number: "II",
    title: "Auditing & Assurance",
    description: "Forensic financial review, statutory audit, and fiduciary reporting. Precision instruments for institutions where the margin for error is zero.",
    icon: <Shield className="h-6 w-6" strokeWidth={1} />,
  },
  {
    number: "III",
    title: "Enforcement",
    description: "Regulatory defence, litigation, and asset recovery. Discreet, resolute representation before tribunals, commissions, and international arbiters.",
...
const ICONS = [
  <Landmark key="1" className="h-6 w-6" strokeWidth={1} />,
  <Shield key="2" className="h-6 w-6" strokeWidth={1} />,
  <Gavel key="3" className="h-6 w-6" strokeWidth={1} />,
  <Scale key="4" className="h-6 w-6" strokeWidth={1} />,
];

function Services({ data }: { data?: PortfolioData }) {
  const services = data?.experience?.length
    ? data.experience.slice(0, 6).map((exp, i) => ({
  ...
  const sectionLabel = data?.experience?.length
    ? `${services.length} engagement${services.length !== 1 ? "s" : ""}`
    : "Three disciplines.";
        ...
        <div className="mb-20 flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
          <div>
            <div className="mb-6 flex items-center gap-4">
              <span className="h-px w-12 bg-gold" />
              <span className="text-[10px] uppercase tracking-[0.4em] text-gold">
                {data?.experience?.length ? "Experience" : "Our Practice"}
              </span>
            </div>
          ...
          <p className="max-w-sm text-sm leading-relaxed text-muted-foreground">
            {data?.experience?.length
              ? "A career built on precision, trust, and enduring results."
              : "Each engagement is led by a named partner. Each brief is treated as a matter of the firm's own reputation."}
          </p>
        </div>
...
function Stats({ data }: { data?: PortfolioData }) {
  const stats = data?.skills?.length
    ? [
        { k: String(data.experience?.length ?? 0), l: "Engagements" },
        { k: String(data.projects?.length ?? 0), l: "Projects delivered" },
        { k: String(data.skills.length), l: "Technical skills" },
        { k: String(data.education?.length ?? 0), l: "Qualifications" },
      ]
    : [
...
function Counsel({ data }: { data?: PortfolioData }) {
  const name = data?.personalInfo?.name ?? "Sir Nathaniel Ashford, KC";
  const role = data?.personalInfo?.role ?? "Senior Partner";
    ...
    <section id="counsel" className="relative bg-charcoal py-32">
      <div className="mx-auto max-w-5xl px-8 text-center">
        ...
        <p className="font-serif text-3xl italic leading-relaxed text-ivory md:text-4xl">
          &ldquo;{bio ?? "We do not chase the fashions of the profession. We measure our work by the century, not the quarter."}&rdquo;
        </p>
        <div className="mt-10 flex flex-col items-center gap-2">
          <span className="h-px w-16 bg-gold" />
          <span className="mt-4 text-[10px] uppercase tracking-[0.4em] text-gold">{name}</span>
          <span className="text-xs text-ivory/50">{role}</span>
        </div>
      </div>
    </section>
  );
...
function Contact({ data }: { data?: PortfolioData }) {
  const email = data?.personalInfo?.email ?? "practice@ashford-vale.co.uk";
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-16 px-8 lg:grid-cols-2">
        <div>
          ...
          <div className="mt-12 space-y-6 text-sm text-ivory/80">
            <div>
              <div className="text-[10px] uppercase tracking-[0.3em] text-gold/70">Email</div>
              <a href={`mailto:${email}`} className="mt-2 block hover:text-gold transition-colors">{email}</a>
            </div>
            {linkedin && (
              <div>
                <div className="text-[10px] uppercase tracking-[0.3em] text-gold/70">LinkedIn</div>
                <a href={linkedin} target="_blank" rel="noopener noreferrer" className="mt-2 block hover:text-gold transition-colors">{linkedin}</a>
              </div>
            )}
            {github && (
              <div>
                <div className="text-[10px] uppercase tracking-[0.3em] text-gold/70">GitHub</div>
                <a href={github} target="_blank" rel="noopener noreferrer" className="mt-2 block hover:text-gold transition-colors">{github}</a>
              </div>
            )}
          </div>
        </div>
...
function Footer({ data }: { data?: PortfolioData }) {
  const name = data?.personalInfo?.name ?? "Ashford & Vale";
  return (
    <footer className="border-t border-white/5 bg-charcoal py-12">
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 px-8 md:flex-row">
        <div className="font-serif text-lg text-ivory">
          {name.includes("&") ? (
            <>{name.split("&")[0]}<span className="text-gold">&</span>{name.split("&")[1]}</>
          ) : (
            <span>{name}</span>
          )}
        </div>
