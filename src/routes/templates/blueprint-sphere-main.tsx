...
export const Route = createFileRoute("/templates/blueprint-sphere-main")({
  head: () => ({
    meta: [
...
function SkillSphere({ skills = SKILLS }: { skills?: { label: string; tag: string }[] }) {
...
function HexGrid({ skills = SKILLS }: { skills?: { label: string; tag: string }[] }) {
  return (
...
function Portfolio({ data }: { data?: PortfolioData }) {
  ...
  const displaySkills = data?.skills?.length
    ? data.skills.map((s) => ({ label: s, tag: "SKILL" }))
    : SKILLS;
    ...
    <div className="relative min-h-screen overflow-x-hidden text-foreground">
      ...
      <header className="sticky top-0 z-40 border-b border-cyan/25 bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3 text-[10px] tracking-[0.3em]">
          <div className="flex items-center gap-3">
            ...
            <span className="text-cyan">{(data?.personalInfo?.name || "ARC//ENG").toUpperCase()}</span>
      ...
      <section id="overview" className="relative min-h-[92vh] overflow-hidden">
        ...
        <div className="relative mx-auto grid min-h-[92vh] max-w-7xl grid-cols-1 items-center gap-12 px-6 py-24 lg:grid-cols-[1.4fr_1fr]">
          ...
          <h1 className="max-w-4xl text-4xl leading-[1.05] font-medium tracking-tight md:text-6xl lg:text-7xl uppercase">
            <span className="text-glow-cyan">{data?.personalInfo?.name?.split(" ")[0] || "AUTONOMOUS"}</span> {data?.personalInfo?.role || "ROBOTICS"},
            <br />
          ...
          <p className="mt-6 max-w-2xl text-sm leading-relaxed text-muted-foreground md:text-base">
            » {data?.personalInfo?.bio || "Building the layers between silicon and cloud — firmware, sensor fusion, fleet orchestration, and the interfaces that keep humans in the loop."}
          </p>
      ...
      <section id="skills" className="relative border-t border-cyan/20 py-24">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="relative rounded-sm border border-cyan/25 bg-card/40 p-6 md:p-12">
            <CornerBrackets />
            {view === "sphere" ? <SkillSphere skills={displaySkills} /> : <HexGrid skills={displaySkills} />}
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-4xl px-6">
          ...
          <div className="relative border border-cyan/40 bg-card/60 p-8 box-glow-cyan md:p-12">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {[
                { k: "MAIL", v: data?.personalInfo?.email || "engineer@arc.systems" },
                { k: "NODE", v: data?.personalInfo?.socials?.github || "github/arc-eng" },
                { k: "FREQ", v: data?.personalInfo?.socials?.linkedin || "linkedin/arc-eng" },
              ].map((c) => (
